...
# a decrease. The genotypic values, in increasing order, are 0, 'd', and 'a',
...
# from a uniform distribution between a/2 and 'a'.
#
...
for (i in 1:NumSim) {
   ...
   for (j in 1:NumLoci) {
      ...
      Qlist[j] <- q
      d <- runif(1, min=a/2, max=a)
      Cad <- Cad + p * q * (a - 2 * d) * (q - p) * (p * a - d * (p - q))
      Va <- Va + 2 * p * q * (p ^ 2 * (a - d) ^ 2 + 2 * p * q * (a - d) * d + q ^ 2 * d ^ 2)
   }
